...
```{r}
  ...
  ocean_url <- "https://erddap.aoos.org/erddap/tabledap/noaa_nos_co_ops_9452210.csv?time%2Cair_temperature%2Csea_water_temperature%2Csea_water_temperature_qc_agg%2Csea_water_temperature_qc_tests%2Cstation&time%3E=2026-02-02T13%3A00%3A00Z"
  ...
  data_2018 <- Juneau[substr(Juneau$Date, 1, 4) == "2018", ]
  data_2018$Month <- as.numeric(substr(data_2018$Date, 6, 7))
  
  data_20_years <- Juneau[substr(Juneau$Date, 1, 2) == "20", ]
...
```{r}
  ...
  hist(data$TAVG..Degrees.Fahrenheit., 
  ...
  for (month in 1:12) {
    hist(data$TAVG..Degrees.Fahrenheit.[data$Month == month], 
...
```{r}
  ...
  for (season in names(seasonal_averages)) {
    ...
    hist(avg_temps, 
         ...
         col = "lightcoral", 
         border = "black")
    var_temp <- var(avg_temps, na.rm = TRUE)
    legend("topright", legend = paste("Var:", round(var_temp, 2)), bty = "n")
  }
```
```{r}
  ...
  # bar plot of avg temps grouped by season
  ggplot(seasonal_samples, aes(x=Season, y=Temp)) +
  geom_boxplot()
...
```{r}
  # histogram of average temp by year for years in the 21st century
  data_20_years$Year <- factor(substr(data_20_years$Date,1,4))

  # add year data
  Juneau$Year <- factor(substr(Juneau$Date,1,4))
  
  # add season data
  Juneau$Month <- factor(substr(Juneau$Date,6,7))
  
  seasons <- list(fall <- fall_months,
                 spring <- spring_months,
                 winter <- winter_months,
                 summer <- summer_months)
  Juneau$Season <- 

  droped_data <- drop_na(Juneau)

  temp <- droped_data$TAVG..Degrees.Fahrenheit.
  
  droped_data |>
    mutate(transf_temp = log(temp)) |>
    ggplot(aes(x = transf_temp)) +
    geom_histogram( ) +
    facet_wrap(~Year)
```
